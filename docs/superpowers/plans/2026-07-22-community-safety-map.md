...
- Tidak ada `alert/confirm/prompt` native browser. Konfirmasi hapus pakai `window.MuterinDialog.confirm`. Form tambah-titik pakai **panel inline** di halaman (bukan dialog), karena ada upload foto.
...
**Interfaces:**
- Consumes: endpoint Task 3, `window.MuterinMap` (`init`, `token`, `fitTo`), `window.MuterinDialog.confirm`.
