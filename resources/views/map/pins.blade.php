<x-app-layout>
    ...
    <div class="p-4 sm:p-6 lg:p-8 max-w-[1400px] mx-auto space-y-6">
        {{-- Hero --}}
        <div class="relative overflow-hidden rounded-[24px] bg-gradient-to-r from-accent to-primary shadow-lift p-6 sm:p-8">
            ...
            <div class="relative flex items-center justify-between gap-4">
                <div class="space-y-2">
                    <span class="inline-flex items-center gap-2 bg-white/15 text-white border border-white/20 font-bold uppercase tracking-[0.15em] text-[10px] px-3 py-1 rounded-full">
                        <x-icon.map-pin class="w-3 h-3"/> {{ $pins->count() }} titik
                    </span>
                    <h1 class="text-2xl sm:text-3xl font-extrabold text-white tracking-tight">Titik Pribadi di Peta</h1>
                    <p class="text-white/80 text-sm max-w-lg">Tandai jalan rawan, jalan sepi, atau momen perjalanan. Klik di peta untuk menambah titik.</p>
                </div>
                <x-icon.map-pin class="w-24 h-24 text-white/10 hidden sm:block"/>
            </div>
        </div>
        ...
        <div class="grid lg:grid-cols-3 gap-6">
            <div class="lg:col-span-2 space-y-3">
                <div class="bg-surface border border-border rounded-2xl shadow-soft p-4 flex flex-wrap items-center gap-3">
                    <label class="text-sm font-semibold text-foreground flex items-center gap-2">
                        Kategori titik:
                        <select id="pin-category" class="rounded-token border border-border bg-surface px-3 py-2 text-sm focus:border-primary focus:ring-2 focus:ring-primary/20">
                    ...
                    <div class="flex items-center gap-3 ms-auto text-xs text-muted-fg">
                        ...
                        <span class="flex items-center gap-1.5"><span class="w-2.5 h-2.5 rounded-full bg-status-green"></span> Sepi</span>
            ...
            <div class="bg-surface border border-border rounded-[24px] shadow-soft overflow-hidden flex flex-col">
                <div class="p-5 border-b border-border bg-muted/40">
                    <h3 class="font-heading font-bold text-foreground">Daftar Titik</h3>
                </div>
                <div class="p-3 space-y-1 overflow-y-auto" style="max-height: 58vh">
                    @php $catLabel = ['hazard' => 'Rawan', 'quiet' => 'Sepi', 'moment' => 'Momen']; @endphp
                    @forelse ($pins as $pin)
                        <div class="p-4 rounded-2xl hover:bg-muted/60 transition flex items-start justify-between gap-3">
                            <div class="min-w-0">
                            ...
                            <button data-delete-pin="{{ $pin->id }}" class="p-1.5 rounded-token text-muted-fg hover:text-accent hover:bg-accent/10 transition shrink-0">
